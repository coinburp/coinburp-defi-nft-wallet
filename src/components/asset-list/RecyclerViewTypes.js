...
export const ViewTypes = {
  ...
  COIN_SAVINGS: {
    calculateHeight: ({ isLast, asset, amountOfRows }) => {
      const isActive =
        asset[0].underlying.symbol && asset[0].supplyBalanceUnderlying;
      return (
        TokenFamilyHeaderHeight +
        (isLast
          ? ListFooter.height +
            (isActive
              ? savingsListAdditionalHeight
              : savingsLastOpenAdditionalHeight)
